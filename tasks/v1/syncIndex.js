...
async function doEverything() {
  let newAnime = await updateIndex(); // 从 Alist 刷新索引，比较耗时
  if (newAnime.length > 0) {
    console.log('[同步] 发现番剧有变动，进行完整刷新...');
    await cutBgmId(); // 分割番剧名和ID  一瞬间
    await insertBgmId(); // 把 anime 表的 bgmId 同步到 bangumi_data 表
    await updateBgmSubjectsData(); // 刷新 bangumi_data 表的 Subjects 数据，同时顺便更新 anime 表的 Poster，也很耗时
...
function updateIndex() { // 更新索引
  ...
  return new Promise(async (resolve, reject) => {
    ...
    if (newAnime.length != 0 && sendQQMessage) {
