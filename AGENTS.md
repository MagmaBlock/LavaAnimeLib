...
pnpm test:full        # 全流程测试（启动 test db → 运行测试 → 停止清理） ⭐ 常规测试唯一推荐方式
pnpm test             # 运行全部测试（需要自行启动 test db，仅用于调试时避免反复启停 db）
...
- `MYSQL_URL` — 数据库连接 URL（`mysql://user:password@host:port/database`）
- `BANGUMI_API_HOST` / `BANGUMI_IMAGE_HOST` — Bangumi API 与封面图域名
- `LOG_LEVEL` / `LOG_DIR` / `LOG_FILE` — 日志配置
- `SECURITY_*` — 安全相关（trustProxy / 登录重试 / 封禁 / Token 过期）
- 存储节点配置已迁移到数据库 `drives` + `connection_configs` 表
- 运行时动态配置（如 Bangumi 缓存策略、首页 Header）存储在数据库 `settings` 表
