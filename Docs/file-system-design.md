...
```
Factory.createDriver(connectionConfig) → FileSystemDriver
  // 根据 connectionConfig.type 实例化对应 Driver
  // 传入 connectionConfig.config 作为连接参数
```
...
将「连接配置」抽取为独立实体，Drive 和 Endpoint 各自引用：

```
connection_configs    — 纯粹的连接配置（host / path / password / type）
     │
     ├── drives.connection_config_id           (1:1)  扫盘用，不对外
     │
     └── drive_endpoints.connection_config_id  (1:N)  用户用，一个 drive 有多个 endpoint
            │
            └── drive_id → drives             (N:1)
```
...
| 操作 | 使用的连接 | 来源 |
|------|-----------|------|
| 全量扫盘 | `drives.connection_config_id` | connection_configs |
| 按需刷新某番剧目录 | `drives.connection_config_id` | connection_configs |
| 构造下载 URL | `endpoint.connection_config_id` | connection_configs |
| 文件是否存在 | 不涉及连接 | file_index (drive_id + path) |

`file_index` 完全不受 endpoint 影响——一个 drive 只维护一份索引。

#### connection_configs 表

| 字段 | 类型 | 说明 |
|------|------|------|
| `id` | INT PK | 自增 |
| `type` | VARCHAR(32) | 驱动类型（"alist"） |
| `config` | JSON | 连接配置，如 `{host, path, password}` |

`type` 决定 Factory 实例化哪个 Driver，`config` 的 JSON 结构由对应 Driver 的 Zod Schema 校验。

#### drives 表改造

**移除**：`host`、`path`、`password`、`type`

**新增**：

| 字段 | 类型 | 说明 |
|------|------|------|
| `connection_config_id` | INT FK | → connection_configs，扫盘专用连接 |

其余字段（`name`、`description`、`banNSFW`、`enabled`、`isDefault`、`sortOrder` 等）不变。
...
| `name` | VARCHAR(100) | 线路名称（"电信"、"海外"） |
| `connection_config_id` | INT FK | → connection_configs，对外连接 |
| `priority` | INT | 优先级，越小越优先，默认 0 |
| `enabled` | TINYINT | 是否启用，默认 1 |

#### 复用同一连接配置

如果一个节点同时是扫盘节点和对外节点，drive 和 endpoint 可以共用同一个 `connection_config_id`，不会产生重复配置。

```
// 共用示例
INSERT INTO connection_configs (type, config)
VALUES ('alist', '{"host":"https://ali.example.com","path":"/Anime","password":""}');
// → id = 1

drives.connection_config_id       = 1   // 扫盘走这个
drive_endpoints.connection_config_id = 1   // 对外也走这个（同一域名）
```
...
- 创建 Drive 时需提供 `connection_config_id`（可先创建 config 再引用，或在接口内联创建）
...
│     for each drive:                                         │
│       config = getConnectionConfig(drive.connection_config_id)│
│       driver = Factory.createDriver(config)                  │
│       scanStart = NOW()                                     │
│       recursiveScan(driver, config.path)  // rootPath 来自 config│
│                                                             │
...
Body: { id: laID, drive: driveId }
  ...
  1. 查找 anime 记录
  2. 查找 drive 记录及对应的 connection_config
...
| POST | `/v2/admin/drive/new` | body 改为 `{connection_config_id, name, ...}` |
| POST | `/v2/admin/drive/update` | 同上 |
| GET | `/v2/admin/drive/all` | 响应增加 `connection_config_id` |
| POST | `/v2/admin/connection-config/new` | **新增**，创建连接配置 |
| POST | `/v2/admin/connection-config/update` | **新增**，更新连接配置 |
| POST | `/v2/admin/connection-config/delete` | **新增**，删除连接配置 |
| GET | `/v2/admin/connection-config/all` | **新增**，列出所有连接配置 |
...
│   │   ├── factory.ts                 # createDriver(connectionConfig) → FileSystemDriver
│   │   └── index.ts                   # barrel export
│   ├── database/schema/
│   │   ├── connection-config.ts        # 新增：connection_configs 表
│   │   ├── drive.ts                    # 修改：移除 type/host/path/password，新增 connection_config_id
│   │   ├── drive-endpoint.ts           # 新增：drive_endpoints 表
...
├── services/v2/
│   ├── admin/
│   │   ├── drive.ts                    # 修改：适配 connection_config_id
│   │   ├── connection-config.ts        # 新增：连接配置 CRUD
...
│   ├── drive/index.ts                  # 修改：适配新结构，增加 endpoint 查询
│
├── controllers/v2/
│   ├── anime/file.ts                   # 修改：新增 refresh 入口，处理 endpoint 参数
│   ├── admin/drive/*.ts                # 修改：适配 connection_config_id
│   ├── admin/connection-config/*.ts    # 新增：连接配置 CRUD
...
│   ├── admin/drive.ts                  # 修改：适配 connection_config_id
│   ├── admin/connection-config.ts      # 新增：连接配置校验
│   ├── admin/drive-endpoint.ts         # 新增：endpoint 校验
│   └── admin/file-index.ts             # 新增：索引查询/刷新参数校验
│
├── routes/v2/
│   ├── anime.ts                       # 修改：新增 /file/refresh 路由
│   └── admin.ts                       # 修改：新增 connection-config + drive-endpoint + file-index 路由组
│
├── tasks/v2/
...
└── tests/
    ...
    │   ├── drive.test.ts               # 修改：适配新结构
    │   ├── connection-config.test.ts   # 新增
    │   └── drive-endpoint.test.ts      # 新增
    └── integration/
        └── remaining.test.ts           # 修改：适配新 API

packages/shared/src/
└── models.ts                           # 修改：新增 ConnectionConfig、EndpointRecord 类型
```
...
### connection_configs 表创建 + drives 表改造

```
-- 1. 创建 connection_configs 表
...
-- 2. 将 drives 现有 host/path/password/type 迁入 connection_configs
INSERT INTO connection_configs (type, config)
SELECT type, JSON_OBJECT('host', host, 'path', path, 'password', password)
FROM drives;

-- 3. drives 新增 connection_config_id 列并回填
ALTER TABLE drives ADD COLUMN connection_config_id INT NULL;
UPDATE drives d
JOIN connection_configs c
  ON c.type = d.type AND c.config = JSON_OBJECT('host', d.host, 'path', d.path, 'password', d.password)
SET d.connection_config_id = c.id;
...
CREATE TABLE drive_endpoints (
  ...
  FOREIGN KEY (connection_config_id) REFERENCES connection_configs(id)
);

-- 5. 对每个已有 drive，用其 config 创建一条默认 endpoint
INSERT INTO drive_endpoints (drive_id, name, connection_config_id, priority)
SELECT id, name, connection_config_id, 0 FROM drives;
```
...
| 步骤 | 内容 | 影响范围 |
|------|------|----------|
| 1 | 新增 `connection_configs` + `drive_endpoints` + `file_index` 三张表 Drizzle schema | schema |
| 2 | 改造 `drives` 表（移除 host/path/password/type，增加 connection_config_id）+ 数据迁移 | schema + migration |
| 3 | 实现 FileSystemDriver 接口 + AlistDriver + Factory（接受 connection_config） | common |
| 4 | 新增 connection_config 管理（CRUD 服务 + Controller + Schema + 路由） | services + controllers |
| 5 | 新增 endpoint 管理（CRUD 服务 + Controller + Schema + 路由） | services + controllers |
| 6 | 实现 FileIndexService（CRUD + TTL + deleted 过滤 + 按路径查询） | services |
| 7 | 实现 Admin 索引管理（list/search/refresh-dir/stats 服务 + Controller） | services + controllers |
| 8 | 重写扫盘任务（全量递归，仅维护 file_index 一致性） | tasks |
| 9 | 重写文件服务（通过 anime.index 路径匹配查询 file_index + endpoint 路由 + 现场解析 + 现场构造 URL） | services |
| 10 | 更新 Controller + Route + 新增 refresh + admin index API | controllers + routes |
| 11 | 适配 Drive 管理（admin CRUD + 前端 Vue：connection_config 选择器 + endpoint 列表） | controllers + web |
| 12 | 更新 shared 包类型 | packages/shared |
| 13 | 更新测试 + 删除废弃代码 | tests |
