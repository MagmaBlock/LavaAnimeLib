...
export function parseFileName(fileName) {
    ...
    return { groupNames, tagedName: reformedFileName, extensionName, episode: thisEpisode }
...
function IsThisWordAGroupName(word, dict) {
    ...
    for (let i in groupDict) { // 遍历此词典的内容
...
function garbageCleaner(word, dict) {
    ...
    for (let i in garbageDict) { // 遍历此词典的内容
...
function getExtensionName(fileName, dict) {
    ...
    for (let i in dict.format) {
        ...
        if (!matched) {
            return { result: thisDict.to, type: thisDict.type, raw: lastOne, trueName: trueName }
        }
    }
