...
export async function testInviteCode(inviteCode) {
    ...
    if (queryResult[0].length) { // 存在结果
        let expired = false
        let used = false
        if (queryResult[0][0].expiration_time !== null) { // 有过期时间设置，判断是否过期
            if (queryResult[0][0].expiration_time < new Date()) { // 过期了
                expired = true
            }
        }
        if (queryResult[0][0].use_time) { // 已经被标记了使用时间
            used = true
        }
